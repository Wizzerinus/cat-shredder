...
class CameraMode(FSM):
    ...
    def _startMouseReadTask(self):
        taskMgr.add(self._mouseReadTask, f"{self._getTopNodeName()}-MouseRead", priority=-29)
    ...
    def _startMouseUpdateTask(self):
        taskMgr.add(self._avatarFacingTask, f"{self._getTopNodeName()}-AvatarFacing", priority=23)
        taskMgr.add(self._mouseUpdateTask, f"{self._getTopNodeName()}-MouseUpdate", priority=40)
