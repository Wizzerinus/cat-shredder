...
from toontown.toonbase.globals.TTGlobalsRender import CameraBitmask, CameraTransparentBitmask, FloorBitmask


class OrbitalCamera(FSM, NodePath, ParamObj):
    notify = directNotify.newCategory("OrbitalCamera")
    ...
    def disableMouseControl(self, pressed, disabledByMouse=True):
        if base.settings.getBool("recenter-camera", True):
            base.cmod.setPreset()
    ...
    def _startCollisionCheck(self):
        ...
        taskMgr.add(self._collisionCheckTask, OrbitalCamera.CollisionCheckTaskName, priority=45)
    ...
    def _collisionCheckTask(self, task=None):
        ...
        if not self.firstPerson:
            if distance < 1.8:
                ...
                self.subject.getGeomNode().show()
        base.cmod.ccPusherTrav.traverse(render)
        return Task.cont
    ...
    def setPresetPos(self, presetIndex, immediate=False):
    ...
    def setCameraPos(self, y, h, p, immediate=False):
        t = (-14 - y) / -12
        ...
        if immediate:
            ...
            Parallel(
                LerpPosInterval(self, 0.45, (self.getX(), self.getY(), z), blendType="easeInOut"),
                LerpHprInterval(self, 0.45, (h, p, 0), blendType="easeInOut"),
            ).start()
    ...
    def _startMouseReadTask(self):
        self._stopMouseReadTask()
        taskMgr.add(self._mouseReadTask, self.TopNodeName + "-MouseRead", priority=-29)
