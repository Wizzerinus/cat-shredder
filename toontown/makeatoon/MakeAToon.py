...
class MakeAToon(StateData.StateData):
    notify = directNotify.newCategory("MakeAToon")
    ...
    def __init__(self, parentFSM, avList, doneEvent, index):
        ...
        for av in avList:
            ...
            self.nameList.append(av.avName)
    ...
    def exit(self):
        base.camLens.setFov(DefaultCameraFov / (4.0 / 3.0))
        self.guiTopBar.hide()
    ...
    def load(self):
        ...
        ee = DirectFrame(
            pos=(-1, 1, 1), frameSize=(-0.01, 0.01, -0.01, 0.01), frameColor=(0, 0, 0, 0.05), state="normal"
        )
    ...
    def squishProp(self, prop):
        if not prop.isEmpty():
            ...
            self.propSquishIval = Sequence(
                Func(self.propSquishActor.play, "propSquish"), Wait(squishDuration), Func(prop.hide)
            )
