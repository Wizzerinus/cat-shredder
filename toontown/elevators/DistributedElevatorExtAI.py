...
class DistributedElevatorExtAI(DistributedElevatorAI.DistributedElevatorAI):
    notify = directNotify.newCategory("DistributedElevatorExtAI")
    ...
    def acceptExiter(self, avId):
        ...
        if seatIndex == None:
            ...
            if self.anyToonsBailed == 0:
                ...
                self.sendUpdate(
                    "emptySlot" + str(seatIndex), [avId, bailFlag, globalClockDelta.getRealNetworkTime(), timeToSend]
                )
            else:
                self.sendUpdate(
                    "emptySlot" + str(seatIndex), [avId, bailFlag, globalClockDelta.getRealNetworkTime(), timeToSend]
    ...
    def enterOpening(self):
        ...
        taskMgr.doMethodLater(
            ElevatorData[ELEVATOR_NORMAL]["openTime"], self.waitEmptyTask, self.uniqueName("opening-timer")
        )
    ...
    def enterClosing(self):
        ...
        taskMgr.doMethodLater(
            ElevatorData[ELEVATOR_NORMAL]["closeTime"], self.elevatorClosedTask, self.uniqueName("closing-timer")
        )
