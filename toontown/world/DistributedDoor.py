...
from toontown.toonbase.globals.TTGlobalsCore import SPHidden, SPRender
from toontown.toonbase.globals.TTGlobalsGUI import getBuildingNametagFont
from toontown.toonbase.globals.TTGlobalsRender import FloorOffset
from toontown.toonbase.globals.TTGlobalsWorld import DoorRejectNames
from toontown.toontowngui import TTDialog
from toontown.world import ZoneUtil


class DistributedDoor(DelayDeletable):
    ...
    def __init__(self, cr):
        ...
        self.fsm = ClassicFSM(
            ...
            [
                State("off", self.enterOff, self.exitOff, ["closing", "closed", "opening", "open"]),
                ...
                State("open", self.enterOpen, self.exitOpen, ["closing", "open"]),
            ],
            "off",
            "off",
        )
        self.fsm.enterInitialState()
        self.exitDoorFSM = ClassicFSM(
            "DistributedDoor_left",
            [
                State("off", self.exitDoorEnterOff, self.exitDoorExitOff, ["closing", "closed", "opening", "open"]),
                ...
                State("open", self.exitDoorEnterOpen, self.exitDoorExitOpen, ["closing", "open"]),
            ],
            "off",
            "off",
        )
    ...
    def avatarEnterDoorTrack(self, avatar, duration):
        ...
        finalPos = avatar.getParent().getRelativePoint(otherNP, Point3(self.doorX, 2, FloorOffset))
        moveHere = Sequence(
        ...
        if avatar.doId == base.localAvatar.doId:
            track.append(
                Sequence(
                    ...
                    Wait(duration * 0.5),
                    Func(avatar.b_setParent, SPHidden),
                )
            )
    ...
    def avatarEnqueueTrack(self, avatar, duration):
        ...
        offset = Point3(self.doorX, back, FloorOffset)
        otherNP = self.getDoorNodePath()
    ...
    def rejectEnter(self, reason):
        message = DoorRejectNames.get(reason)
        if message:
    ...
    def avatarExitTrack(self, avatar, duration):
        ...
        track.append(PosHprInterval(avatar, Point3(-self.doorX, 0, FloorOffset), VBase3(179, 0, 0), other=otherNP))
        track.append(Func(avatar.setParent, SPRender))
        if avatar.doId == base.localAvatar.doId:
            track.append(
                PosHprInterval(
                    base.camera, VBase3(-self.doorX, 5, avatar.getHeight()), VBase3(180, 0, 0), other=otherNP
                )
            )

        if avatar.doId == base.localAvatar.doId:
            finalPos = base.render.getRelativePoint(otherNP, Point3(-self.doorX, -6, FloorOffset))
        else:
            finalPos = base.render.getRelativePoint(otherNP, Point3(-self.doorX, -3, FloorOffset))
    ...
    def exitCompleted(self):
        ...
        base.localAvatar.d_setParent(SPRender)
