...
class BossHealthBar:
    bossBarColors = (
        Vec4(0.2, 0.5, 0.2, 1),
        Vec4(0.6, 0.6, 0.3, 1),
        Vec4(0.7, 0.55, 0.15, 1),
        Vec4(0.45, 0.15, 0.15, 1),
        Vec4(0.2, 0.2, 0.2, 1),
    )
    ...
    def __init__(self, dept):
        ...
        self.gui.setScale(1, 1, 0.75)
        self.gui.setColorScale(0.4, 0.4, 0.4, 1)
        self.gui.setTexture(self.bossBarFrameBg)
        self.gui.setTransparency(1)
        self.damageBar = DirectWaitBar(
            relief=None,
            scale=(0.195, 0, 0.077),
            value=100,
            pos=(-0.005, 0, 0.0015),
        ...
        self.bossBar = DirectWaitBar(
            ...
            range=100,
            frameColor=(0.1, 0.1, 0.1, 1),
            barColor=(0.2, 0.2, 0.9, 1),
            text="0 / 0",
            text_scale=(0.16, 0.32),
            text_fg=(0.9, 0.9, 0.9, 1),
            text_align=TextNode.ACenter,
            text_pos=(0, -0.1),
            text_shadow=(0, 0, 0, 1),
            text_font=getCompetitionFont(),
        )
        self.gui.hide()
        self.bossBar.hide()
        self.damageBar.hide()
    ...
    def __drainDamageBar(self, task=None):
        ...
        self.drainIval = Sequence(
            LerpFunctionInterval(self.__updateDrainBar, fromData=start, toData=goal, duration=0.5),
    ...
    def __applyNewColor(self, currColor, condition):
        if self.bossBar and currColor != 3 and currColor != 4 and currColor != 5:
            if self.healthRatio > self.colorThresholds[0]:
                condition = 0
            elif self.healthRatio > self.colorThresholds[1]:
                condition = 1
            ...
            numeratorRatioAmt = self.colorThresholds[condition - 1] if condition > 0 else 1
