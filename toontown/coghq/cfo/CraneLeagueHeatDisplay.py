...
from toontown.toonbase.globals.TTGlobalsGUI import getCompetitionFont, getSuitFont

LOWEST_HEAT_H_VALUE = 80
HIGHEST_HEAT_H_VALUE = 0

LOWEST_HEAT_S_VALUE = 0.41
HIGHEST_HEAT_S_VALUE = 0.70
...
class CraneLeagueHeatDisplay:
    FLAME_POS = (-0.53, 0, 0.74)
    HEAT_NUM_POS = (-0.48, 0.72)
    MODIFIERS_TEXT_POS = (-0.64, 0, 0.6)
    ...
    def __init__(self):
        ...
        self.flame_image = OnscreenImage(
            parent=self.frame, image="phase_10/maps/heat.png", pos=self.FLAME_POS, scale=0.05
        )
        self.flame_image.setTransparency(TransparencyAttrib.MAlpha)

        self.heat_number = OnscreenText(
            parent=self.frame,
            text="500",
            style=3,
            fg=self.calculate_color(),
            align=TextNode.ALeft,
            scale=0.1,
            pos=self.HEAT_NUM_POS,
            font=getSuitFont(),
        )

        self.hover_button = DirectButton(parent=self.frame, pos=(-0.43, 0, 0.74), scale=(1.5, 1, 0.5))
        ...
        self.modifiers_desc.setCardColor(0.2, 0.2, 0.2, 0.75)
    ...
    def calculate_color(self):
        ...
        r, g, b = colorsys.hsv_to_rgb(h, s, V_HEAT_VALUE)
        return r, g, b, 1
