from toontown.coghq.cfo.GeneralCFOGlobals import TreasureTypes
...
SIDE_CRANE_POSHPR = [(81, -315, 0, -90, 0, 0), (160, -315, 0, 90, 0, 0)]
HEAVY_CRANE_POSHPR = [(120, -280, 0, 180, 0, 0), (120, -350, 0, 0, 0, 0)]
...
class CFORuleset:
    def __init__(self):
        ...
        self.CFO_ATTACKS_BASE_DAMAGE = {
            TTGlobalsBosses.BossCogElectricFence: 1,  # The actual bump
            TTGlobalsBosses.BossCogSwatLeft: 5,  # Swats from bumping
            TTGlobalsBosses.BossCogSwatRight: 5,
            TTGlobalsBosses.BossCogSlowDirectedAttack: 15,  # Gear throw
            TTGlobalsBosses.BossCogAreaAttack: 20,  # Jump
        }

        # How much should attacks be multiplied by by the time we are towards the end?
        self.CFO_ATTACKS_MULTIPLIER = 3
        # should multiplier gradually scale or go up by integers?
        # False means 1x then 2x then 3x, True gradually increases
        ...
        self.LOW_LAFF_BONUS_THRESHOLD = (
            25  # How much laff or less should a toon have to be considered for a low laff bonus?
        )
        ...
        self.REVIVE_TOONS_UPON_DEATH = (
            True  # Should we revive a toon that dies after a certain amount of time? (essentially a stun)
        )
        ...
        self.POINTS_PENALTY_GO_SAD = (
            -50
        )  # Point deduction for dying (can happen multiple times if revive setting is on)
        ...
        self.TREASURE_POINT_PENALTY_FLAT_RATE = (
            1  # How much should we deduct? set to 0 or less to make it 1 to 1 with laff gained
        )
    ...
    def update_lists(self):
        ...
        self.TREASURE_STYLES = [
            [TreasureTypes.ICE_CREAM, TreasureTypes.STARFISH],
            [TreasureTypes.STARFISH, TreasureTypes.FLOWER],
            [TreasureTypes.MUSIC_NOTE, TreasureTypes.SNOWFLAKE],
            [TreasureTypes.SNOWFLAKE, TreasureTypes.ZZZS],
        ]
    ...
    def validate(self):
        ...
        self.MIN_SAFE_IMPACT = min(self.MIN_SAFE_IMPACT, 0.95)
        self.MIN_DEHELMET_IMPACT = min(self.MIN_DEHELMET_IMPACT, 0.95)
        self.MIN_GOON_IMPACT = min(self.MIN_GOON_IMPACT, 0.95)
        self.SIDECRANE_IMPACT_STUN_THRESHOLD = min(self.SIDECRANE_IMPACT_STUN_THRESHOLD, 0.95)
...
class CFORulesetModifierBase(object):
    ...
    def asStruct(self):
        return [self.MODIFIER_ENUM, self.tier]
...
class ModifierCFOHPDecreaser(CFORulesetModifierBase):
    ...
    def _perc_decrease(self):
        ...
        # Don't let it go higher than 99% or lower than 65%
        return min(99, max(65, self.tier * 5 + 50))
...
class ModifierDesafeImpactIncreaser(CFORulesetModifierBase):
    ...
    def getDescription(self):
        perc = self._perc_increase()
        return (
            "Increases the impact required to remove the CFO's helmet by %(color_start)s+"
            + str(perc)
            + "%%%(color_end)s"
        )
...
class ModifierGeneralImpactDecreaser(CFORulesetModifierBase):
    ...
    def getDescription(self):
        return (
            "Decreases general impact required by %(color_start)s-"
            + str(self.PERC_REDUCTION)
            + "%%%(color_end)s for goons and safes"
        )
...
class ModifierGeneralImpactIncreaser(CFORulesetModifierBase):
    ...
    def getDescription(self):
        return (
            "Increases general impact required by %(color_start)s+"
            + str(self.PERC_REDUCTION)
            + "%%%(color_end)s for goons and safes"
        )
...
class ModifierGoonDamageInflictIncreaser(CFORulesetModifierBase):
    ...
    def getDescription(self):
        ...
        return (
            "Increases damages inflicted to the CFO from goons by %(color_start)s+"
            + str(self._perc_increase())
            + "%%%(color_end)s"
        )
...
class ModifierSafeDamageInflictIncreaser(CFORulesetModifierBase):
    ...
    def getDescription(self):
        return (
            "Increases damages inflicted to the CFO from safes by %(color_start)s+"
            + str(self._perc_increase())
            + "%%%(color_end)s"
        )
...
class ModifierTreasureRNG(CFORulesetModifierBase):
    ...
    def getDescription(self):
        return (
            "Treasures have a %(color_start)s-"
            + str(self._get_decrease())
            + "%%%(color_end)s chance to drop from stunned goons"
        )
...
class ModifierUberBonusIncreaser(CFORulesetModifierBase):
    ...
    def getDescription(self):
        return (
            "Points gained from UBER BONUS increased by %(color_start)s+"
            + str(self._perc_increase())
            + "%%%(color_end)s"
        )
...
class ModifierMillionMomentum(CFORulesetModifierBase):
    ...
    def getDescription(self):
        return (
            "The CFO has a %(color_start)smillion HP%(color_end)s, "
            "but toons gain %(color_start)spermanent damage buffs %(color_end)swhen inflicting damage!"
        )
...
def getModifiersOfType(mod_type_enum):
    ...
    for mod_subclass in list(CFORulesetModifierBase.MODIFIER_SUBCLASSES.values()):
        # If the type of this class matches the param we passed in
        if mod_type_enum == mod_subclass.MODIFIER_TYPE:
