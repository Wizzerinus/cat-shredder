...
from toontown.coghq.cfo import CraneLeagueGlobals, DistributedCashbotBossGoon, GeneralCFOGlobals
...
class DistributedCashbotBossCrane(DistributedObject.DistributedObject, FSM.FSM):
    ...
    def __init__(self, cr):
        ...
        self.magnetSoundInterval = Parallel(
            SoundInterval(self.magnetOnSfx), Sequence(Wait(0.5), Func(base.playSfx, self.magnetLoopSfx, looping=1))
        )
    ...
    def announceGenerate(self):
        ...
        cn.addSolid(cs)
        cn.setIntoCollideMask(WallBitmask)
        ...
        cn.addSolid(cs)
        cn.setIntoCollideMask(PieBitmask)
        self.controls.attachNewNode(cn)

        self.boss.craneArm.copyTo(self.crane)
    ...
    def __activatePhysics(self):
        if not self.physicsActivated:
            for an, _anp, cnp in self.activeLinks:
    ...
    def setupCable(self):
        ...
        cn.setFromCollideMask(CashbotBossObjectBitmask)
    ...
    def startFlicker(self):
        ...
        for i in range(4):
            t = float(i) / 3.0 - 0.5
            node = self.boss.lightning.copyTo(self.gripper)
            node.setScale(random.choice([1, -1]), 1, 5)
            node.setZ(random.uniform(-5, -5.5))
            node.flattenLight()
            node.setTwoSided(1)
            node.setBillboardAxis()
            node.setScale(random.uniform(0.5, 1.0))
            if t < 0:
                node.setX(t - 0.7)
            else:
                node.setX(t + 0.7)
            node.setR(-20 * t)
            node.setP(random.uniform(-20, 20))
            self.lightning.append(node)
    ...
    def grabObject(self, obj):
        ...
        self.boss.craneStatesDebug(
            doId=self.doId,
            content="pre-Grabbing object %s, currently holding: %s"
        ...
        obj.lerpInterval = Parallel(
            obj.posInterval(GeneralCFOGlobals.CashbotBossToMagnetTime, Point3(*obj.grabPos)),
            obj.quatInterval(GeneralCFOGlobals.CashbotBossToMagnetTime, VBase3(obj.getH(), 0, 0)),
        ...
        if self.avId == base.localAvatar.doId and not self.magnetOn:
            # We got a late grab.  Grab it, then immediately drop it.
            self.releaseObject()

        self.boss.craneStatesDebug(
            doId=self.doId,
            content="post-Grabbing object %s, currently holding: %s"
            % (obj.getName(), self.heldObject.getName() if self.heldObject else "Nothing"),
        )
    ...
    def dropObject(self, obj):
        ...
        obj.lerpInterval = Parallel(
            obj.quatInterval(GeneralCFOGlobals.CashbotBossFromMagnetTime, VBase3(obj.getH(), 0, 0), blendType="easeOut")
        )
        obj.lerpInterval.start()
    ...
    def releaseObject(self):
        ...
        if self.boss:
            self.boss.craneStatesDebug(
                doId=self.doId,
                content="pre-Releasing object, currently holding: %s"
                % (self.heldObject.getName() if self.heldObject else "Nothing"),
            )
        ...
        if self.heldObject:
            ...
            if obj.state == "Grabbed":
                ...
                # owner.
                obj.demand("LocalDropped", base.localAvatar.doId, self.doId)

        if self.boss:
            self.boss.craneStatesDebug(
                doId=self.doId,
                content="post-Releasing object, currently holding: %s"
                % (self.heldObject.getName() if self.heldObject else "Nothing"),
            )
    ...
    def d_sendCablePos(self):
        ...
        self.sendUpdate(
            ...
            [
                ...
                self.crane.getY(),
                self.arm.getH()
                + self.armMaxH,  # don't let this number go negative, can't send negative # over an update
                links,
                timestamp,
            ],
        )
    ...
    def exitControlled(self):
        ...
        if self.avId == base.localAvatar.doId:
            ...
            if self.cr:
                ...
                if place and hasattr(place, "fsm") and place.fsm.getCurrentState().getName() == "crane":
