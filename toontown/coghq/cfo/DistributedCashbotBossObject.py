...
class DistributedCashbotBossObject(DistributedSmoothNode.DistributedSmoothNode, FSM.FSM):
    ...
    def __init__(self, cr):
        ...
        self.collisionNode.setIntoCollideMask(PieBitmask | WallBitmask | CashbotBossObjectBitmask | CameraBitmask)
        self.collisionNode.setFromCollideMask(PieBitmask | FloorBitmask)
    ...
    def __hitFloor(self, entry):
        if self.state in ("Dropped", "LocalDropped"):
            self.d_hitFloor()
            self.demand("SlidingFloor", base.localAvatar.doId)
    ...
    def __hitBoss(self, entry):
        if (self.state in ("Dropped", "LocalDropped")) and self.craneId != self.boss.doId:
    ...
    def __getCraneAndObject(self, avId):
        if self.boss and self.boss.cranes is not None:
            for crane in list(self.boss.cranes.values()):
                if crane.avId == avId:
    ...
    def exitLocalDropped(self):
        if self.newState not in ("SlidingFloor", "Dropped"):
    ...
    def enterDropped(self, avId, craneId):
        ...
        if self.avId == base.localAvatar.doId:
            self.activatePhysics()
            self.startPosHprBroadcast(period=0.05)
